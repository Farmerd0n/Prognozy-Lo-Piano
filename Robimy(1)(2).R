...
#' ## Definitions: 
# 
#' Harmonized Indices of Consumer Prices (HICPs) are designed for international comparisons of consumer price inflation. HICP is used by e.g. the European Central Bank for monitoring inflation in the Economic and Monetary Union and for the assessment of inflation convergence as required under Article 121 of the Treaty of Amsterdam. The data taken into account in calculating HICP inflation comes from household budget expenditures and national accounts. 
# 
#  
# 
#' The explanatory variables for the model were selected based on a review of scientific literature. Empirical studies on inflation in the euro area indicate that HICP may be influenced by factors such as unemployment, public spending on pensions and healthcare, budget balance, residential construction activity, industrial orders, consumer confidence indicators, and trade balance. These variables were included as potential determinants of inflation within the econometric model. 
# 
#  
# 
#' Unemployment data was sourced from GUS. In years where two versions (“a” and “b”) were available (2002, 2020, and 2021), version “b” was selected, as it incorporates information from the more recent Agricultural Census. 
# 
#' Figures related to healthcare and pensions reflect general price levels compared to the previous month. These figures were also obtained from GUS. 
# 
#' The budget balance was calculated by dividing the government’s net gains or losses by its total income. We chose this method of presenting budget surpluses and deficits to offer an unusual perspective on this date point. Data was sourced from GUS. 
# 
#' New housing refers to the number of newly completed flats made available for sale on the housing market. This data was provided by GUS. 
# 
#' Industry orders at current prices indicate month-on-month changes in the value of orders placed. The data was sourced from GUS. 
# 
#  
# 
#' ## Cited works
...
#  
# 
#' Unemployment: Fröhling, A., & Lommatzsch, K. (2011). Output sensitivity of inflation in the euro area: Indirect evidence from disaggregated consumer prices (Discussion Paper No. 2011,25). Deutsche Bundesbank. 
# 
#' Pensions: Checherita-Westphal, C. (Ed.). (2022). Public wage and pension indexation in the euro area: An overview (Occasional Paper Series No. 299). European Central Bank. 
# 
#' Healthcare: Kovačević, M., & Jakšić, D. (2021). Forecasting medical inflation in the European Union using the ARIMA model. Public Sector Economics, 45(4), 445–472. 
# 
#' Budget Balance: Staehr, K., Tkačevs, O., & Urke, K. (2024). Fiscal performance under inflation and inflation surprises: Evidence from fiscal reaction functions for the euro area. Review of World Economics, 160(4), 1477–1504. 
# 
#' New housing: Tomal, M. (2021). Identification of house price bubbles using robust methodology: Evidence from Polish provincial capitals. Empirical Economics, 62(3), 1461–1490. 
# 
#' Industry orders: European Central Bank. (2014, March). Impact of services and non-energy industrial goods prices on the recent decline in HICP inflation. ECB Monthly Bulletin, March 2014, pp. 60–62 
# 
#' Current Consumer Confidence Indicator: Central Bank of Ireland. Inflation expectations of euro area consumers and firms. Retrieved from https://www.centralbank.ie 
# 
#  
# 
#' Trade balance: Landolfo, L. (2007). Modeling the impact of external factors on the euro area’s HICP and real economy: A focus on pass-through and the trade balance (ECB Working Paper No. 789). European Central Bank. 
#libraries
if (!require("readxl")) {
...
#' diff check for non stationary variables
...
#   theme_minimal()
#' With different models tested we found out that parameters were not good enough so we decided to try to eliminate some of variables
...
#' FIXING UNEMPLOYMENT VIF - we tried to change VIF values but the model witout it was better
...
#' We judged that the old model was visually better suiting

#'# Predictor measures. 
#' The way in which data for the variables will be estimated, to be later put into the DYNLM model (50% wartość z t-12, 25% wartość z t-24, 15% średnia wartość z poprzedzającego roku, 10% średnia wartość z roku 2 lata temu)
...
#' # Data for forecast
#' Using aforemtioned method to estimate variables for the next 12 months
vars_to_forecast <- c(
  "Healthcare", 
  "Pensions", 
  "Unemployment", 
...
#' # Prediction block
#'Early step of preparing data for the forecast
